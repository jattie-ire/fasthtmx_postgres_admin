...
import re
import shutil
from config import DB_CONFIG, config, get_audit_trail_table_name
...
def get_tables_with_display_names():
    """Get list of available tables with their display names
    
    Returns:
        List of dicts: [{"table_name": "...", "display_name": "..."}, ...]
    """
    from config import get_table_display_name
...
def create_audit_table():
    ...
    audit_table_name = get_audit_trail_table_name()
    ...
    try:
        ...
        (
            index integer NOT NULL GENERATED BY DEFAULT AS IDENTITY ( INCREMENT 1 START 1 MINVALUE 1 MAXVALUE 2147483647 CACHE 1 ),
            userid text COLLATE pg_catalog."default",
            datetime timestamp with time zone,
            reference text COLLATE pg_catalog."default",
            action text COLLATE pg_catalog."default",
            CONSTRAINT {audit_table_name}_pkey PRIMARY KEY (index)
        )
        ...
        conn.commit()
        print(f"✓ Audit trail table '{audit_table_name}' verified/created")
...
def get_primary_key(table_name: str) -> str:
    ...
    try:
        ...
        # Query information_schema for primary key
        cursor.execute(f"""
            SELECT a.attname
            FROM pg_index i
        ...
        for pk_name in ['id', 'index', f'{table_name}_id']:
            cursor = conn.cursor()
        ...
        print(f"WARNING: Could not find primary key for table '{table_name}'")
        return 'id'  # Default fallback
        
    except Exception as e:
        print(f"ERROR getting primary key for table '{table_name}': {e}")
        return 'id'  # Default fallback


def log_audit_event(userid: str, reference: str, action: str) -> bool:
    """Log an audit event to the audit trail table
    
    Args:
        userid: The user performing the action
        reference: Usually formatted as 'table_name:id=primary_key_value'
    ...
    try:
        ...
        # Insert audit log
        cursor.execute(f"""
            INSERT INTO {audit_table_name} (userid, datetime, reference, action)
            VALUES (%s, %s, %s, %s)
        """, (userid, now, reference, action))
        
        conn.commit()
        cursor.close()
        return_db_connection(conn)
        
        print(f"✓ Audit logged: {action} by {userid} on {reference}")
        return True
...
def delete_user(username: str) -> bool:
    """Delete a user from the fastapi_users table
    ...
    try:
        ...
        # Delete user
        cursor.execute(f"""
            DELETE FROM {FASTAPI_USERS_TABLE}
            WHERE "user" = %s
        """, (username,))
        ...
        if rows_deleted > 0:
            ...
            print(f"WARNING: User '{username}' not found for deletion")
            return False

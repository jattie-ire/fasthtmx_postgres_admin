...
✅ **Log Viewer** — Browse, search, and analyze log files generated by scripts  
...
- **Connection Pooling**: Optimized PostgreSQL performance with ThreadedConnectionPool (minconn=2, maxconn=10)
  - **Auto-recovery** — Proper connection lifecycle management prevents pool exhaustion
  - **Graceful error handling** — Failed connections returned to pool instead of discarded
...
- **Log Viewer**: Browse and analyze script logs with syntax highlighting and smart pattern recognition

### Recent Fixes & Improvements
- **Connection Pool Exhaustion Prevention** (v1.1+): Fixed critical bug where database connections were permanently removed from pool instead of returned, causing "PoolError" after 10 requests. All database operations now properly return connections to the pool.
- **Log File Discovery & Display** (v1.1+): New `/logs` page with automatic log discovery, organization by script, and formatted viewing
...
Browse and analyze log files generated by your scripts with syntax-highlighted, searchable interface:

```toml
[scripts]
"Background Run" = "/path/to/run.sh"

[scripts.log_config]
# Configure where logs are stored and how to find them
"Background Run" = { directory = "../logs", pattern = "debug_log_*.log", display = true }

[scripts.log_display]
# Global log viewer settings
limit = 5           # Show the N most recent logs per script (default: 5)
max_width = 1200    # Maximum width of log viewer modal in pixels (default: 1200)
no_wrap = true      # Disable line wrapping with horizontal scrollbar (default: true)
```

**Features**:
- **Automatic discovery** — Finds log files matching configured patterns
- **Organized by script** — Logs grouped by which script generated them
- **Advanced syntax highlighting**:
  - ISO 8601 timestamps → blue
  - File and function names (e.g., `app.py:main:process`) → orange
  - Numeric values (integers & floats) → cyan
  - ERROR/CRITICAL/FATAL → red with background
  - WARNING/WARN → gold with background
  - INFO → green with background
  - DEBUG/TRACE → purple/gray with background
  - Key=value pairs (e.g., `color=red`) → light cyan
- **Configurable display**:
  - `max_width` — Control modal width for different screens
  - `no_wrap` — Enable/disable horizontal scrollbar for long lines
- **Large file support** — Efficiently handles multi-megabyte log files
- **Responsive modal viewer** — Click any log to open in full-screen modal with rich formatting

**URL**: `http://localhost:8000/logs` (requires `run_scripts` permission)

**Configuration**:
- `[scripts.log_config]` — Per-script log locations and glob patterns
- `[scripts.log_display]` — Global settings:
  - `limit` — Max logs per script to display
  - `max_width` — Modal width in pixels
  - `no_wrap` — Line wrapping behavior
- Logs sorted newest-first by epoch filename

**Implementation**:
- Backend: `list_all_logs()` discovers and sorts logs; `/api/logs` returns organized JSON; `/api/logs/{file}` reads content
- Frontend: Custom log highlighting with regex pattern matching; DOM-based rendering for safety

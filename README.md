...
[scripts.log_display]
# Show the N most recent logs per script
limit = 5
```
...
- **Syntax highlighting** — Timestamps, log levels (ERROR/WARNING/INFO/DEBUG), key=value pairs highlighted with distinct colors
...
- `[scripts.log_display]` — Global settings (limit: max logs per script to display)
